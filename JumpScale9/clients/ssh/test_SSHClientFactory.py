...
class TestSSHClientFactory(unittest.TestCase):
    ...
    def test_get_async_ssh_default(self):
        """
        ...
        ssh_client_factory = SSHClientFactory.SSHClientFactory()
        ssh_client_factory.reset()
        ssh_client = ssh_client_factory.get_async()
    ...
    @pytest.mark.ssh_factory
    def test_get_async_ssh_client_cache(self):
        """
        Checks if j.client.ssh.get_async() caching works properly
        """
        with patch('JumpScale9.clients.ssh.AsyncSSHClient.AsyncSSHClient') as AsyncSSHClient:
            from JumpScale9.clients.ssh import SSHClientFactory
            ssh_client_factory = SSHClientFactory.SSHClientFactory()
            ssh_client_factory.reset()
            ssh_instance_id_first = id(ssh_client_factory.get_async())
            ssh_instance_id_second = id(ssh_client_factory.get_async())
            assert ssh_instance_id_first == ssh_instance_id_second


    @pytest.mark.ssh_factory
    def test_get_async_ssh_client_not_cache(self):
        """
        ...
        ssh_client_factory = SSHClientFactory.SSHClientFactory()
        ssh_client_factory.reset()
        ssh_instance_id_first =  ssh_client_factory.get_async(usecache=False)
        ssh_instance_id_second = ssh_client_factory.get_async(usecache=False)
        assert not id(ssh_instance_id_first) == id(ssh_instance_id_second)
