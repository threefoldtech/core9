...
class ZeroRobot:
    ...
    def generateSchemas(self):
        ...
        path0 = "%s/schemaspy" % self._path
        j.sal.fs.removeDirTree(path0)
        for name in [j.sal.fs.getBaseName(item) for item in j.sal.fs.listFilesInDir(path)]:
            cmd = "cd %s;mkdir -p %s;protoc -I=schemas --python3_out=schemaspy/ schemas/%s" % (
                self._path, "%s/schemaspy" % self._path, name)
            j.do.execute(cmd)
