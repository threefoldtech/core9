...
class ConfigFactory():
    ...
    def _findConfigDirParent(self,path,die=True):
        ...
        while curdir.strip()!="":
            if j.sal.fs.exists("%s/.jsconfig"%curdir) and j.sal.fs.exists("%s/.git"%curdir):
                print("found jsconfig dir in: %s"%curdir)
                break
            #look for parent
        ...
        if curdir.strip()!="":
            return curdir    
        if die:
            raise RuntimeError("Could not find config dir as parent of:'%s'"%path)
        else:
            return None
                ...
                for key, path in res.items():
                    ...
                    if key.startswith("config_"):
                        if j.sal.fs.exists(checkpath):
                            self._path = path
                            print("found jsconfig dir in: %s"%self._path)
                            return self._path
            else:
                return self._path
    ...
    def _get_for_obj(self, factoryclassobj,template,ui=None, instance="main",data={}):
        ...
        if not hasattr(factoryclassobj, '__jslocation__') or factoryclassobj.__jslocation__ is None or factoryclassobj.__jslocation__ is "":
            raise RuntimeError("__jslocation__ has not been set on class %s"%factoryclassobj.__class__)
        ...
        if key not in self._cache:            
            sc = Config(instance=instance, location=location, template=template ,ui=ui,data=data)
            self._cache[key]=sc
